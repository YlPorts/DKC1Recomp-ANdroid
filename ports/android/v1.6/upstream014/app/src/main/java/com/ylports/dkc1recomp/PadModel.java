...
public final class PadModel {
    ...
    private static final class Finger {final int origin;int current;Finger(int origin){this.origin=origin;this.current=origin;}}
    ...
    /**
     * Update one contact without discarding its origin. A thumb that begins on
     * Y keeps Y held for the lifetime of the contact; rolling it onto B adds B
     * and rolling back releases only B. This is implemented here rather than
     * in Android gesture APIs so drawing and game input share one state model.
     */
    public void touch(int id,float x,float y){
        int current=hit(x,y)&4095;
        Finger finger=fingers.get(id);
        if(finger==null){if(fingers.size()>=16)return;finger=new Finger(current);fingers.put(id,finger);}
        if((finger.origin&Y)!=0)current|=Y;
        finger.current=current;
    }
    public void release(int id){fingers.remove(id);}public void clear(){fingers.clear();}
    public int state(){int n=0;for(Finger f:fingers.values())n|=f.current;if((n&(UP|DOWN))==(UP|DOWN))n&=~(UP|DOWN);if((n&(LEFT|RIGHT))==(LEFT|RIGHT))n&=~(LEFT|RIGHT);return n;}
}