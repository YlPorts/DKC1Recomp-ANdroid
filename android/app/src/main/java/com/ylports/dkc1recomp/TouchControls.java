...
public final class TouchControls extends View {
    ...
    @Override public boolean onTouchEvent(MotionEvent e){
        ...
        pad.clear();
        if(visibleButtons&&action!=MotionEvent.ACTION_UP)for(int i=0;i<e.getPointerCount();i++){
            if(action==MotionEvent.ACTION_POINTER_UP&&i==e.getActionIndex())continue;pad.touch(e.getPointerId(i),e.getX(i),e.getY(i));}
