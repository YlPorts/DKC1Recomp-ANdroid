...
## Follow-up: fine-scroll crossings without a rebase

The broader September 6 [camera audit](WIDESCREEN_CAMERA_AUDIT.md) found the same missing native guard row during ordinary Coral Capers scrolling. The guard capture now also runs on currently calibrated fine-Y=7 frames under `DKC1_WS_SCROLL_REBASE=1`. Across 50 movement branches this removes 124 terrain misses, with exact native-center and guest-memory hashes and three deterministic repeats. The normal app is updated for the next launch; the user's ongoing game was preserved. The report records actual-window/layer evidence, the exact-state recipe and remaining fresh-entry/full-matrix limits.
